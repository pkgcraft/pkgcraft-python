...
from pkgcraft.dep import Blocker, Cpv, Operator, PkgDep, SlotOperator, Version, VersionWithOp
from pkgcraft.eapi import EAPIS, Eapi
from pkgcraft.error import InvalidCpv, InvalidPkgDep
...
class TestPkgDep:
    def test_init(self):
        ...
        assert repr(a).startswith("<PkgDep 'cat/pkg' at 0x")

        # all fields -- extended EAPI default allows repo deps
        a = PkgDep("!!>=cat/pkg-1-r2:0/2=[a,b,c]::repo")
        assert a.category == "cat"
        assert a.package == "pkg"
        ...
        assert repr(a).startswith("<PkgDep '!!>=cat/pkg-1-r2:0/2=[a,b,c]::repo' at 0x")

        # explicitly specifying an official EAPI fails
        for eapi in ("8", EAPIS["8"]):
            with pytest.raises(InvalidPkgDep):
                PkgDep("cat/pkg::repo", eapi)

        # unknown EAPI
        with pytest.raises(ValueError, match="unknown EAPI"):
            PkgDep("cat/pkg", "nonexistent")

        # invalid EAPI type
        with pytest.raises(TypeError):
            PkgDep("cat/pkg", object())
    ...
    def test_valid(self, toml_data):
        attrs = []
        for attr, val in inspect.getmembers(PkgDep):
        ...
        for entry in toml_data["pkgdep.toml"]["valid"]:
            s = entry["dep"]
            ...
            for eapi in EAPIS.values():
                if eapi in passing_eapis:
                    ...
                    assert str(a) == s
                    assert repr(a).startswith(f"<PkgDep {s!r} at 0x")
                else:
                    with pytest.raises(InvalidPkgDep, match=f"invalid dep: {re.escape(s)}"):
                        PkgDep(s, eapi)

    def test_invalid(self, toml_data):
        for s in toml_data["pkgdep.toml"]["invalid"]:
            for eapi in EAPIS.values():
                with pytest.raises(InvalidPkgDep, match=f"invalid dep: {re.escape(s)}"):
                    PkgDep(s, eapi)
    ...
    def test_intersects(self, toml_data):
        ...
        for d in toml_data["pkgdep.toml"]["intersects"]:
    ...
    def test_sort(self, toml_data):
        for d in toml_data["pkgdep.toml"]["sorting"]:
            expected = [PkgDep(s) for s in d["sorted"]]
    ...
    def test_hash(self, toml_data):
        for d in toml_data["version.toml"]["hashing"]:
            s = {PkgDep(f"=cat/pkg-{x}") for x in d["versions"]}
            length = 1 if d["equal"] else len(d["versions"])
            assert len(s) == length

    def test_cached(self):
        l = [PkgDep.cached("cat/pkg") for _ in range(1000)]
        assert len(l) == 1000

    def test_pickle(self):
        a = PkgDep("=cat/pkg-1-r2:0/2=[a,b,c]")
        b = pickle.loads(pickle.dumps(a))
        assert a == b
